...
2. **JWT Tokens**
   ...
   - Token includes user identification (username)
...
1. **Repository Pattern** (via Service Layer)
   - `BaseService` provides CRUD operations
   - Specific services (`HotelService`, `RateService`) extend base functionality
...
3. **Domain-Driven Design**
   - Clear separation between Models (ORM), Schemas (DTOs), and Services
...
1. **Component Composition**
   ...
   - Reusable UI elements (cards, forms, headers)

2. **Context API**
   - Centralized authentication state
   - Avoids prop drilling

3. **Custom Hooks** (Implicit)
...
- **Choice**: SQLite for development/demo
- **Rationale**: Simplifies setup, no external dependencies
- **Trade-off**: Limited concurrency, fewer features
- **Future**: PostgreSQL recommended for production

### Float vs Decimal for Currency
- **Choice**: Float for rates
- **Rationale**: Simpler serialization, sufficient for demo
- **Trade-off**: Potential rounding issues
- **Future**: Use Decimal or integer (cents) for production

### Single Admin User
- **Choice**: Seeded admin account
- **Rationale**: Focuses on domain features, not auth complexity
- **Trade-off**: No user management, roles, or registration
- **Future**: Implement full user management system

### Vanilla CSS vs Tailwind/CSS-in-JS
- **Choice**: Vanilla CSS with variables
- **Rationale**: Lightweight, full control, no build overhead
- **Trade-off**: More manual styling work
- **Future**: Could adopt utility-first framework if needed

---

## Future Enhancements

1. **Multi-tenancy**: Support multiple hotel chains
2. **Reservations**: Add booking and availability management
3. **Reporting**: Analytics dashboard for rates and occupancy
4. **Real-time Updates**: WebSocket support for live rate changes
5. **Advanced Auth**: Role-based access control, OAuth integration
6. **Database Migration**: Move to PostgreSQL for production
7. **API Versioning**: Explicit version management beyond v1
8. **Caching**: Redis for frequently accessed data
9. **File Uploads**: Hotel images and documents

---

## Conclusion

The Basic Hotel Platform demonstrates a clean, maintainable architecture with clear separation of concerns. The layered approach (Client → API → Service → Data) ensures scalability and testability while keeping the codebase approachable for developers.
